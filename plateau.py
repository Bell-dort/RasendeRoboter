...
class plateau():
    ...
    def elaborationPlateau1(self, L):
        for i in range(4):
            listeChangement = []
    ...
    def elaborationPlateau2(self, liste):
        for i in range(8):
            for j in range(8):
                self.plat[i][j] = (self.listePlaque[liste[0]])[i][j]
        for i in range(8, 16):
            for j in range(8):
                self.plat[i][j] = (self.listePlaque[liste[1]])[i][j]
        for i in range(8):
            for j in range(8, 16):
                self.plat[i][j] = (self.listePlaque[liste[2]])[i][j]
        for i in range(8, 16):
            for j in range(8, 16):
                self.plat[i][j] = (self.listePlaque[liste[3]])[i][j]
